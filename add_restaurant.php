   ...
   if(isset($_POST['email'])){
      ...
      $admin = 'admin';
      $sql = $mysqli -> prepare("SELECT * FROM users WHERE email = ? AND password = ? AND type = ?");
      $sql -> bind_param("sss", $email, $password, $admin);
      $sql -> execute();
      $sql = $sql -> get_result();
      ...
      if($user){
         if(isset($_POST['restaurant_name'])){
            ...
            if(isset($restaurant['restaurant_name'])) {
               die ('Restaurant name already in use, Please choose another name');
            }else{
